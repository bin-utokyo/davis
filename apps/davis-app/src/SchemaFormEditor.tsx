...
type FormSection = {
  bind: string; widget?: string; input?: string; title?: string; labels?: Record<string, string>;
...
function NestEditor({ section, value, candidates, onChange }: { section: FormSection; value: unknown[]; candidates: string[]; onChange: (value: unknown[]) => void }) {
  ...
  return <><div className="subsection-heading"><div><h3>{section.title ?? section.bind}</h3><p>各選択肢を重複なく1つのnestへ入れます．</p></div><button className="secondary" onClick={() => onChange([...nests, { name: `nest_${nests.length + 1}`, alternatives: [], dissimilarity: { initial: 0.8 } }])}>Nestを追加</button></div>{!nests.length && <div className="empty-state">2つ以上のnestを追加してください．</div>}{nests.map((nest, index) => { const dissimilarity = asObject(nest.dissimilarity); const fixed = "fixed" in dissimilarity; return <div className="nest-row" key={index}><input value={String(nest.name ?? "")} onChange={(event) => patch(index, { name: event.target.value })} placeholder="motorized" /><AlternativePicker candidates={candidates} selected={(nest.alternatives as unknown[] | undefined)?.map(String) ?? []} onChange={(selected) => patch(index, { alternatives: selected })} /><select value={fixed ? "fixed" : "initial"} onChange={(event) => patch(index, { dissimilarity: event.target.value === "fixed" ? { fixed: 1 } : { initial: 0.8 } })}><option value="initial">推定</option><option value="fixed">固定</option></select><input type="number" min="0.05" max="1" step="0.05" value={String(dissimilarity[fixed ? "fixed" : "initial"] ?? (fixed ? 1 : 0.8))} onChange={(event) => patch(index, { dissimilarity: { [fixed ? "fixed" : "initial"]: numberOrEmpty(event.target.value) } })} /><button className="text-button danger-text" onClick={() => onChange(nests.filter((_, current) => current !== index))}>削除</button></div>; })}</>;
}
