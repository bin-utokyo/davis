...
export function SchemaFormEditor({ definition, inputs, config, onAddSources, onAddCatalog, onInputChange, onConfigChange }: {
  ...
  return <>
    <div className="subsection-heading"><div><h3>{t("入力データ")}</h3><p>{t("入力と結合はモデルではなくDavis共通のtable bindingとして保存します．")}</p></div></div>
    ...
    {sections.map((section) => {
      ...
      return <div className="manifest-section" key={section.bind}>
        ...
        {!knownWidget(widget) && !extension && <YamlFallback section={section} value={value} onChange={(next) => update(section.bind, next)} reason={extensionId ? (t("UI extension {id}を読み込めません．").replace("{id}", extensionId)) : t("widget {id}はこのDesktopに未実装です．").replace("{id}", widget)} />}
      </div>;
    })}
    {alternatives && <p className="hint">{t("候補値")}: {alternatives.values.length} {t("件")}，{t("取得行数")}: {alternatives.rows_sampled}{alternatives.truncated ? ` (${t("上限付きsample")})` : ""}</p>}
  </>;
}
...
function ExtensionSection({ section, extension, schema, value, config, inputs, onChange }: { section: FormSection; extension: UiExtension; schema?: JsonSchema; value: unknown; config: Record<string, unknown>; inputs: Record<string, FormInput | undefined>; onChange: (value: unknown) => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all(Object.entries(section.context ?? {}).map(async ([name, declaration]) => {
      try { return [name, await resolveContext(declaration, config, inputs, t), undefined] as const; }
...
async function resolveContext(declaration: ContextProvider, config: Record<string, unknown>, inputs: Record<string, FormInput | undefined>, t: (value: string) => string): Promise<unknown> {
  ...
  if (!input) throw new Error(t("入力 {id} が未選択です．").replace("{id}", declaration.input ?? ""));
  ...
  if (typeof alias !== "string" || !alias) throw new Error(t("列参照 {id} が未設定です．").replace("{id}", declaration.column_from ?? ""));
  ...
  if (!binding || !source) throw new Error(t("列 {column} を入力 {input} から解決できません．").replace("{column}", alias).replace("{input}", declaration.input ?? ""));
...
function InputBindingEditor({ slot, metadata, input, onAdd, onAddCatalog, onChange }: { slot: string; metadata: InputPresentation; input?: FormInput; onAdd: () => void; onAddCatalog: () => void; onChange: (input: FormInput | undefined) => void }) {
  ...
  return <article className="binding-editor"><div className="binding-heading"><div><strong>{metadata.title ?? slot}</strong><code>{slot}</code>{metadata.description && <p>{metadata.description}</p>}</div><div className="source-actions"><button className="secondary" onClick={onAdd}>{t("ローカルCSV")}</button><button className="secondary" onClick={onAddCatalog}>Davis Catalog</button></div></div>
    {!sources.length && <div className="empty-state compact-empty">{t("CSVを1つ以上選択してください．")}</div>}
    {sources.map((source) => <div className="binding-source" key={source.id}><div><strong>{source.id}</strong><small>{source.origin?.kind === "catalog" ? `${source.origin.dataset_id} / ${source.origin.file_id}` : source.path}</small></div><div className="profile-summary">{source.origin?.kind === "catalog" && <span>Catalog</span>}<span>{source.profile.encoding}</span><span>{source.profile.rows_sampled} {t("行")}</span><span>{source.profile.columns.length} {t("列")}</span></div><button className="text-button danger-text" onClick={() => remove(source.id)}>{t("削除")}</button></div>)}
    {sources.length > 1 && input && <><label className="wide-label"><span>{t("基準データ")}</span><select value={input.base} onChange={(event) => onChange({ ...input, base: event.target.value })}>{sources.map((source) => <option key={source.id}>{source.id}</option>)}</select></label>
      {base && sources.filter((source) => source.id !== input.base).map((source) => { const join = { ...defaultJoin(), ...input.joins[source.id] }; return <div className="join-row" key={source.id}><strong>{base.id}</strong>
        <select value={join.leftOn} onChange={(event) => updateJoin(source.id, { leftOn: event.target.value })}><option value="">{t("左キー")}</option>{base.profile.columns.map((column) => <option key={column.name}>{column.name}</option>)}</select><span className="join-mark">=</span><strong>{source.id}</strong>
        <select value={join.rightOn} onChange={(event) => updateJoin(source.id, { rightOn: event.target.value })}><option value="">{t("右キー")}</option>{source.profile.columns.map((column) => <option key={column.name}>{column.name}</option>)}</select>
        ...
        <select value={join.how} onChange={(event) => updateJoin(source.id, { how: event.target.value as FormJoin["how"] })}><option value="left">left</option><option value="inner">inner</option></select><label className="check-label"><input type="checkbox" checked={join.allowUnmatched} onChange={(event) => updateJoin(source.id, { allowUnmatched: event.target.checked })} />{t("未照合を許可")}</label></div>; })}</>}
  </article>;
}

function SectionTitle({ section }: { section: FormSection }) { const { t } = useI18n(); return <div className="subsection-heading"><div><h3>{section.title ?? section.bind}</h3><p>{section.description ?? <><code>{section.bind}</code>{t("としてAnalysis Planへ保存します．")}</>}</p></div></div>; }
function ColumnMap({ section, schema, value, input, onChange }: { section: FormSection; schema?: JsonSchema; value: Record<string, unknown>; input?: FormInput; onChange: (value: Record<string, unknown>) => void }) {
  ...
  return <><SectionTitle section={section} /><div className="role-grid">{names.map((name) => { const property = schema?.properties?.[name]; const acceptsMany = schemaAcceptsArray(property); return <label key={name}><span>{section.labels?.[name] ?? name}{required.includes(name) ? " *" : ` (${t("任意")})`}</span>{acceptsMany ? <AlternativePicker candidates={columns.map((column) => column.alias)} selected={Array.isArray(value[name]) ? (value[name] as unknown[]).map(String) : typeof value[name] === "string" ? [String(value[name])] : []} onChange={(selected) => onChange(selected.length ? { ...value, [name]: selected } : without(value, name))} /> : <select value={typeof value[name] === "string" ? value[name] as string : ""} disabled={!input} onChange={(event) => onChange(event.target.value ? { ...value, [name]: event.target.value } : without(value, name))}><option value="">{t("列を選択")}</option>{columns.map((column) => <option key={column.alias} value={column.alias}>{column.label}</option>)}</select>}</label>; })}</div></>;
}
function UtilityTerms({ section, value, input, candidates, onChange }: { section: FormSection; value: unknown[]; input?: FormInput; candidates: string[]; onChange: (value: unknown[]) => void }) {
  ...
  return <><div className="subsection-heading"><div><h3>{section.title ?? section.bind}</h3><p>{section.description ?? t("parameterと効用へ入れる列を指定します．")}</p></div><button className="secondary" onClick={() => onChange([...terms, { parameter: `beta_${terms.length + 1}`, column: "", ...(section.show_coefficient ? { coefficient: 1 } : {}) }])}>{t("termを追加")}</button></div>{!terms.length && <div className="empty-state">{t("効用termを追加してください．")}</div>}
...
function NestEditor({ section, value, candidates, onChange }: { section: FormSection; value: unknown[]; candidates: string[]; onChange: (value: unknown[]) => void }) {
  const { t } = useI18n();
  const nests = value.map(asObject); function patch(index: number, change: Record<string, unknown>) { onChange(nests.map((nest, current) => current === index ? clean({ ...nest, ...change }) : nest)); }
  const estimateLabel = section.labels?.estimate ?? t("推定"); const fixedLabel = section.labels?.fixed ?? t("固定");
  return <><div className="subsection-heading"><div><h3>{section.title ?? section.bind}</h3><p>{section.description ?? t("各選択肢を重複なく1つのnestへ入れます．")}</p></div><button className="secondary" onClick={() => onChange([...nests, { name: `nest_${nests.length + 1}`, alternatives: [], scale_mu_d: { initial: 0.8 } }])}>{t("Nestを追加")}</button></div>{!nests.length && <div className="empty-state">{t("2つ以上のnestを追加してください．")}</div>}{nests.map((nest, index) => { const scaleMuD = asObject(nest.scale_mu_d); const fixed = "fixed" in scaleMuD; const valueLabel = section.labels?.[fixed ? "fixed_value" : "estimate_value"] ?? t(fixed ? "固定値" : "推定初期値"); return <div className="nest-row" key={index}><input value={String(nest.name ?? "")} onChange={(event) => patch(index, { name: event.target.value })} placeholder="motorized" /><AlternativePicker candidates={candidates} selected={(nest.alternatives as unknown[] | undefined)?.map(String) ?? []} onChange={(selected) => patch(index, { alternatives: selected })} /><select aria-label={section.labels?.mode ?? t("推定または固定")} value={fixed ? "fixed" : "initial"} onChange={(event) => patch(index, { scale_mu_d: event.target.value === "fixed" ? { fixed: 1 } : { initial: 0.8 } })}><option value="initial">{estimateLabel}</option><option value="fixed">{fixedLabel}</option></select><input aria-label={valueLabel} title={valueLabel} type="number" min="0.05" max="1" step="0.05" value={String(scaleMuD[fixed ? "fixed" : "initial"] ?? (fixed ? 1 : 0.8))} onChange={(event) => patch(index, { scale_mu_d: { [fixed ? "fixed" : "initial"]: numberOrEmpty(event.target.value) } })} /><button className="text-button danger-text" onClick={() => onChange(nests.filter((_, current) => current !== index))}>{t("削除")}</button></div>; })}</>;
}
function ParameterSettings({ section, schema, value, config, onChange }: { section: FormSection; schema?: JsonSchema; value: Record<string, unknown>; config: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void }) {
  ...
  return <><SectionTitle section={section} />{!names.length && <div className="empty-state">{t("先に効用termを追加してください．")}</div>}{names.map((name) => { const settings = asObject(value[name]); return <div className="parameter-row" key={name}><strong>{name}</strong>{fields.map((field) => <label key={field}><span>{field}</span><input type="number" value={settings[field] === undefined ? "" : String(settings[field])} placeholder={t("任意")} onChange={(event) => update(name, field, event.target.value)} /></label>)}</div>; })}</>;
}
function AutoSection({ section, schema, value, onChange }: { section: FormSection; schema?: JsonSchema; value: unknown; onChange: (value: unknown) => void }) { const { t } = useI18n(); return <><SectionTitle section={section} />{schema && canAutoRender(schema) ? schema.type === "object" && schema.properties ? <div className="field-grid compact-grid"><AutoFields schema={schema} value={asObject(value)} onChange={onChange} /></div> : <div className="field-grid"><label><span>{schema.title ?? section.title ?? section.bind}</span><ScalarField schema={schema} value={value ?? schema.default} onChange={onChange} /></label></div> : <YamlFallback section={section} value={value} onChange={onChange} reason={t("自由形式または配列の設定です．")} />}</>; }
...
function YamlFallback({ section, value, onChange, reason }: { section: FormSection; value: unknown; onChange: (value: unknown) => void; reason: string }) {
  const { t } = useI18n();
  const [text, setText] = useState(""); const [error, setError] = useState(""); useEffect(() => { invoke<string>("render_yaml_value", { value: value ?? null }).then(setText).catch((reason) => setError(String(reason))); }, [value]);
  ...
  return <div className="section-fallback"><p>{reason} {t("このsectionだけYAMLで編集できます．")}</p><textarea value={text} onChange={(event) => setText(event.target.value)} aria-label={`${section.bind} YAML editor`} /><button className="secondary" onClick={apply}>{t("このsectionへ反映")}</button>{error && <div className="error">{error}</div>}</div>;
}
function AlternativePicker({ candidates, selected, onChange }: { candidates: string[]; selected: string[]; onChange: (value: string[]) => void }) { const { t } = useI18n(); const [search, setSearch] = useState(""); const [open, setOpen] = useState(false); const visible = candidates.filter((candidate) => candidate.toLocaleLowerCase().includes(search.toLocaleLowerCase())).slice(0, search ? 50 : 8); function toggle(candidate: string) { onChange(selected.includes(candidate) ? selected.filter((item) => item !== candidate) : [...selected, candidate]); } return <div className="alternative-picker"><div className="alternative-control">{selected.map((item) => <button type="button" className="selected-tag" key={item} onMouseDown={(event) => event.preventDefault()} onClick={() => toggle(item)}>{item} ×</button>)}<input value={search} onFocus={() => setOpen(true)} onBlur={() => setOpen(false)} onChange={(event) => setSearch(event.target.value)} placeholder={t(selected.length ? "検索…" : candidates.length ? "値を検索" : "候補列を先に指定")} disabled={!candidates.length} /></div>{open && !!visible.length && <div className="alternative-menu">{visible.map((candidate) => <button type="button" className={selected.includes(candidate) ? "selected" : ""} key={candidate} onMouseDown={(event) => event.preventDefault()} onClick={() => toggle(candidate)}>{candidate}</button>)}</div>}</div>; }
