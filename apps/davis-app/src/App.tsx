...
function RunComparisonView({ runs }: { runs: RunComparison[] }) {
  ...
  return <div className="run-comparison"><div className="subsection-heading"><div><h3>Run comparison</h3><p>{t("artifact名ではなくManifestに記録されたmetrics・parameters profileを照合しています．")}</p></div></div>
    ...
    {parameterNames.length > 0 && <ComparisonTable title="Parameters (estimate)" names={parameterNames} runs={runs} value={(run, name) => run.parameters[name]?.estimate} unbounded />}
  </div>;
}
function ComparisonTable({ title, names, runs, value, unbounded = false }: { title: string; names: string[]; runs: RunComparison[]; value: (run: RunComparison, name: string) => unknown; unbounded?: boolean }) {
  return <div className="comparison-table"><h4>{title}</h4><div className={`result-table${unbounded ? " unbounded" : ""}`}><table><thead><tr><th>Name</th>{runs.map((run) => <th key={run.entry.run.request.run_id}>{run.entry.label || run.entry.plan_name}<small>{run.entry.run.request.component.id}</small></th>)}</tr></thead><tbody>{names.map((name) => <tr key={name}><th>{name}</th>{runs.map((run) => <td key={run.entry.run.request.run_id}>{formatResultValue(value(run, name))}</td>)}</tr>)}</tbody></table></div></div>;
}
...
function ResultPreview({ definition, preview }: { definition: ResultDefinition; preview: ArtifactPreview }) {
  ...
  if (definition.widget === "table" && isTablePreview(preview.content)) {
    ...
    return <article className="result-view"><div className="result-view-title"><h3>{definition.title}</h3><span>{definition.artifact}</span></div><div className={`result-table${definition.profile === "parameters" ? " unbounded" : ""}`}><table><thead><tr>{visibleColumns.map(({ column }) => <th key={column}>{parameterColumnLabel(column, definition.profile, locale)}</th>)}</tr></thead><tbody>{preview.content.rows.map((row, rowIndex) => <tr key={rowIndex}>{visibleColumns.map(({ column, index }) => <td key={index}>{formatParameterCell(column, row[index], definition.profile)}{definition.profile === "parameters" && column === "t_value" && significanceIndex >= 0 && row[significanceIndex] ? <sup>{row[significanceIndex]}</sup> : null}</td>)}</tr>)}</tbody></table>{preview.content.truncated && <p className="hint">{t("先頭200行を表示しています．")}</p>}</div></article>;
  }
