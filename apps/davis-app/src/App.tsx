...
function ResultPreview({ definition, preview }: { definition: ResultDefinition; preview: ArtifactPreview }) { const { t } = useI18n(); return <article className="result-view"><div className="result-view-title"><h3>{definition.title}</h3><span>{definition.artifact}</span></div>{definition.widget === "table" && isTablePreview(preview.content) ? <div className="result-table"><table><thead><tr>{preview.content.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{preview.content.rows.map((row, index) => <tr key={index}>{row.map((cell, cellIndex) => <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table>{preview.content.truncated && <p className="hint">{t("先頭200行を表示しています．")}</p>}</div> : <KeyValuePreview content={preview.content} />}</article>; }
