...
Each file Manifest records the date of the `davis push` that changed it to its current Object ID as `updated_at`. The date is preserved when the Object ID is unchanged, and a dataset's last-updated date is derived from the latest date among its files. The Web Catalog displays these dates in the dataset list, file selection, and file details.
