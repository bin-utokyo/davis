...
- [2. インストール](#2-インストール)
    - [2.1. 前提条件](#21-前提条件)
    - [2.2. インストール手順](#22-インストール手順)
- [3. 初回セットアップ](#3-初回セットアップ)
    - [3.1. Google Cloudプロジェクトの作成](#31-google-cloudプロジェクトの作成)
    - [3.2. Google Drive APIの有効化](#32-google-drive-apiの有効化)
    - [3.3. OAuth 2.0 クライアントIDの発行](#33-oauth-20-クライアントidの発行)
    - [3.4. テストユーザーの追加](#34-テストユーザーの追加)
- [4. データセットの利用方法](#4-データセットの利用方法)
    - [4.1. 利用可能なデータセットの一覧表示](#41-利用可能なデータセットの一覧表示)
    - [4.2. データセットの詳細情報を確認](#42-データセットの詳細情報を確認)
    - [4.3. データセットのダウンロード](#43-データセットのダウンロード)
- [5. その他](#5-その他)
    - [5.1. ヘルプの表示](#51-ヘルプの表示)
- [6. サポート](#6-サポート)
...
- **DVCのインストール**: [DVC公式サイト](https://dvc.org/doc/install) の手順に従ってインストールしてください。
    - `uv tool install --with 'pydrive2>=1.21.3' 'dvc[gdrive]'` でインストールできます。PyDrive2 の指定は、Google Drive バックエンドと暗号化ライブラリの互換性を保つために必要です。
...
1. **Google Drive フォルダID**
   - 運営側から提供されるフォルダIDを入力してください。
2. **Google Cloud Client ID**
3. **Google Cloud Client Secret**

これらの情報は、データが保管されているGoogle Driveにアクセスするために必要です。

Google Cloud Client IDとClient Secretは、ご自身でGoogle Cloud Consoleから取得する必要があります。
以下に取得方法を詳しく説明します。
