...
| P2 | 一部完了 | ComponentManifest，AnalysisPlan，RunRequest，RunResult，local・run artifact Input Resolver，言語非依存process Runner，標準MNL，2段階NL参考component，node状態・link加法型RL参考component，CSV transform参考component，推定時の複数source table binding，単一・複合keyによる宣言的join・列選択，CSV／Parquet出力，Manifestによる出力artifact検証，CSV inspection，成果物・log・digest記録，非破壊package検証・scaffold，local component installer，公式component registry client，決定的bundle・registry生成，release添付workflow | catalog入力，宣言的filter・group，追加source間の連鎖join，disk-backed大量データ処理，registry署名・trust policyは未実装です．公式artifactは次のrelease tag公開時に添付されます |
| P3 | 一部着手 | 低頻度のCLI更新通知，Tauri desktop prototype，clone非依存project workspace，CLIからのDesktop導入・起動，cwd非依存software store，platform別software registry，入力カード内CSV inspection，MNL／NL／RL共通の`davis.ui/v1` composition renderer，入力slot共通`table-binding` widget，JSON Schema `auto` form，section単位YAML fallback，旧UI契約adapter，component同梱sandbox UI extension，複数CSVのstar型join，選択肢候補combobox，AnalysisPlan作成・Form再編集・YAML fallback，preview・保存・検証・実行，`run.label`／Plan名・実行日時・一意suffixによる可読なrun ID，Manifest駆動result viewと結果directory表示 | OS標準のapp bundle／shortcut生成，共有UI extension registry，完全custom editor，複合key・連鎖join editor，managed runtime，run比較，高度な結果可視化，論文authoring，汎用fmt・viz，GC，履歴通知等は未実装です |
