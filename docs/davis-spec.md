...
| P3 | 一部着手 | 低頻度のCLI更新通知，Tauri desktop prototype，GUIからのCSV確認，複数CSVのstar型join・MNL role・線形効用termによるAnalysisPlan作成，生成YAMLのpreview・保存・検証・実行，既存plan実行，結果directory表示 | 既存planのForm再編集，複合key・連鎖join editor，配布用app installer，結果可視化・比較，論文authoring，汎用fmt・viz，GC，履歴通知等は未実装です |
