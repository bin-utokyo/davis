...
5. MNLは固定機能ではなく，新しいモデルコンポーネントを作る参考実装とする
6. 初期Webは，実データごとの`*.schema.yaml`を横断検索・絞り込みできるダウンロードカタログに限定する
7. 接続は少数の安定した境界に限定し，モデル内部は共通化しない
8. データ取得後の整形，推定，結果整理はローカル実行を基本とするが，入口をCLIへ固定しない
9. 通常の研究では，共通処理をDavisへ任せ，主にモデルコンポーネントだけを変更できるようにする
10. 利用者が記述する`project.yaml`はMVPに設けず，Davisが実行条件を`run.json`へ保存する
11. `feature/davis-gui`はそのまま採用せず，サービス分離，実験比較，共通結果，スキーマ検証を取り入れる
12. CLIは最初の参照クライアントであり，Davisの中核や最終的な利用形態ではない
13. Git-nativeなデータ管理・カタログ基盤をDavisの土台とし，研究実行層はその上へ接続する
14. Gitはmetadataと履歴，大容量実体はオブジェクトストレージで管理する
15. `davis_core`にはCLI，Web，モデル固有の処理を含めない
16. 夏の学校公式環境では，データの登録・更新・公開を運営に限定し，参加者は閲覧・ダウンロードだけを行う
17. Runtimeは公式カタログを必須とせず，ローカルデータや利用者組織のサーバーを入力にできる
18. ローカル・外部入力は，明示的な公開操作なしに公式ストレージへuploadしない
