...
初期段階では，対話型ウィザードやノーコード化を中心に置きません．Webの責務は，実データファイルごとの`*.schema.yaml`を使った閲覧，検索，絞り込み，ダウンロードに限定します．データ取得後の整形，推定，可視化はCLIを正式な入口とします．
...
10. 標準係数表を出力するモデルについて，CLIから共通のHTML・CSV・JSONを生成できる
...
| `davis_app` | GUI需要が確認された場合のローカル実行環境 | P2以降 |
...
| 境界 | 形式 | 初期利用者 | 将来の利用者 |
| --- | --- | --- | --- |
| カタログAPI | HTTP＋OpenAPI | Web，CLI | 外部ポータル，Notebook，別組織のサービス |
| アプリケーションAPI | Rustの中核ユースケース | CLI，APIサーバー | PCアプリ，サーバージョブ |
| モデル実行API | JSON＋Parquetのプロセス契約 | ローカルモデル | コンテナ，WASI，遠隔ランナー |

Webのためだけの検索ロジック，CLIのためだけのダウンロードロジックを作りません．`davis_catalog`と`davis_core`にユースケースを置き，Web APIとCLIは薄い入口にします．一方，細かな内部関数まで遠隔APIとして公開せず，実際に別プロセス・別サービスとの接続が必要な境界だけを版管理します．
...
| PC | Tauriを将来検討 | CLI経路の安定後に必要性を判断する |
