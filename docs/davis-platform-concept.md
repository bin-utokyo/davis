...
ただし，DVCを利用者経路から外すのは，現行`davis-cli`の全ダウンロード対象をR2経路から取得し，ハッシュ一致を確認した後に限ります．代表データだけの動作確認ではMVP完了としません．
...
MVPのリリース条件として，現行`davis-cli`から取得できるデータを1件も欠落させません．現在のマニフェストが表すデータセットID，階層指定，DVCファイル，関連文書を互換性の基準として固定します．

移行は次の順序で行います．

1. 現行マニフェストから全ダウンロード対象とサイズ・ハッシュのbaselineを生成する
2. 全対象をR2へコピーし，新`dataset.yaml`とカタログ索引へ対応付ける
3. 参加者として新APIから全対象のDownloadPlanを生成できることを検証する
4. リリース監査で全ファイルを新経路から取得し，サイズとSHA-256をbaselineと照合する
5. 監査が1件でも失敗した場合はMVPをリリースせず，現行`davis-cli`とGoogle Drive／DVC経路を維持する

開発中は`migrating`や`legacy-only`を表示できますが，MVPリリース時に現行取得対象が`legacy-only`のまま残ることは許可しません．新CLIを公開する場合は，少なくとも次の互換試験を行います．
...
コマンド出力の文字列や内部実装を完全一致させる必要はありませんが，取得できるデータ，関連文書，出力先指定，整合性検証は機能低下させません．
...
- 入力エラーと未収束の説明を整える
- 現行の全データセットIDとディレクトリ指定についてDownloadPlanを検証する
...
P0で削ってよいものは高度なデザイン，大容量向けPCアプリ，任意プラグイン実行，サーバー推定です．削ってはいけないものは現行全データのダウンロード互換性，PC内実行，入力検証，収束表示，結果契約，ハッシュ，ライセンス表示，参加者認証，秘密情報の分離です．全データのR2転送と取得監査が完了するまでMVPをリリースしません．移行中も現行`davis-cli`を利用可能な状態に保ちます．
...
- 現行マニフェストの全対象について，データ公開からカタログ反映，DownloadPlan生成，ダウンロード，SHA-256照合まで
...
| R2移行でデータが欠落する | 現行CLIからの機能低下 | baseline，全件取得監査，ハッシュ照合，旧経路の継続 |
...
8. ADR-0008: 現行全データのダウンロード互換性をP0のリリース条件とし，MNL readinessとは分離するか
...
2. 現行マニフェスト，256個のDVCメタデータ，177個の列スキーマから互換baselineと新カタログを作る棚卸しツールを作る
