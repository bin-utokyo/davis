...
[English](davis-component-authoring_en.md)
...
機密データをAI serviceへ送信しないでください．入力列は，実データを含まないschemaファイルに列名，型，単位，欠損値の扱い，カテゴリ値の意味等を記載して説明します．列名自体が機密情報を含み得る場合は，匿名化した列名と意味の対応表を使用してください．sampleが必要な場合も，実在する値や個人を特定できる値を含まない，小さな合成データを使用します．
...
目的: (行いたい推定や計算)
入力: (file名，形式，機密情報を含まないschema．実データは渡さない)
実行時に変更したい設定: (説明変数等)
出力: (必要な表や指標)
...
このframeworkの最終検証では，Davis開発の会話履歴やsource codeを見ていないブラウザAIを使います．AIへ渡してよいものは，このガイド，作りたいモデルの要件，実データを含まないschema，機密情報を含まない小さな合成sampleだけです．既存componentを模写させることは必須にしません．
